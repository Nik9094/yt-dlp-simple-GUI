...
The program uses the configuration file's location `AppData\Roaming\yt-dlp\config.txt`. To change it, you have to modify the `--config-location` parameter in line 46 and the `SpostaConfig` (MoveConfig) subroutine.
